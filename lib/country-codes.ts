...
export function getFlagUrl(iso2: string, width = 20) {
  return `https://flagcdn.com/w${width}/${iso2.toLowerCase()}.png`;
}

export function getCountryByIso(iso2: string) {
  return COUNTRY_CODES.find((country) => country.iso2 === iso2);
}

export function getDialCodeByIso(iso2: string) {
  return (
    COUNTRY_CODES.find((country) => country.iso2 === iso2)?.dialCode ??
    COUNTRY_CODES.find((country) => country.iso2 === DEFAULT_COUNTRY_ISO)
      ?.dialCode ??
    "+44"
  );
}

export function formatPhoneNumber(countryIso: string, phone: string) {
  const trimmed = phone.trim();
  if (!trimmed) return "";

  const dialCode = getDialCodeByIso(countryIso);
