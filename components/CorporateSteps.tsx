...
const steps = [
  ...
  },
  {
    ...
    desc: "Monitor trips in real time with full visibility on usage.",
  },
  {
    num: 5,
    title: "Manage & Report",
...
export default function CorporateSteps() {
  ...
  const thisColor = useTransform(
    scrollYProgress,
    [0, 0.05],
    ["#D1D5DB", "#000000"],
  );

  const isColor = useTransform(
    scrollYProgress,
    [0.05, 0.1],
    ["#D1D5DB", "#000000"],
  );

  const rideHailingColor = useTransform(
    scrollYProgress,
    [0.05, 0.15],
    ["#D1D5DB", "#9CA3AF"],
  );
  ...
  const infraColor1 = useTransform(
    scrollYProgress,
    [0.1, 0.15],
    ["#D1D5DB", "#000000"],
  );
  const infraColor2 = useTransform(
    scrollYProgress,
    [0.15, 0.2],
    ["#D1D5DB", "#000000"],
  );
  const infraColor3 = useTransform(
    scrollYProgress,
    [0.2, 0.25],
    ["#D1D5DB", "#000000"],
  );
  const infraColor4 = useTransform(
    scrollYProgress,
    [0.25, 0.3],
    ["#D1D5DB", "#000000"],
  );
  ...
  return (
    <section
      id="corporate-how-it-works"
      ref={containerRef}
      className="relative h-[450vh]  w-full bg-white font-display"
    >
      <div className="sticky top-0 flex h-screen w-full flex-col justify-center pt-24 md:pt-32 overflow-hidden">
        <div className="mx-auto w-full max-w-7xl px-6 md:px-12 py-6 sm:py-12">
          <h2 className="mb-8 md:mb-12 text-[32px] sm:text-[40px] md:text-[56px] lg:text-[72px] font-medium leading-[1.1] tracking-tight">
            ...
            <motion.span style={{ color: infraColor1 }}>
              infrastructure
            </motion.span>{" "}
          ...
          <div className="relative mt-4 sm:mt-12 w-full flex-grow">
            ...
            >
              {steps.map((step, index) => {
                ...
                const endPoint = 0.3 + (index + 1) * (0.7 / steps.length);
