...
import { ChevronRight, ChevronLeft, Shield, FileText, LayoutList } from "lucide-react";
      ...
      <div className={`fixed top-0 left-0 right-0 z-50 flex justify-center transition-all duration-500 ${scrolled ? "md:px-4 md:pt-4 md:sm:pt-6" : "px-0 pt-0"}`}>
        ...
        >
          ...
          <div className="hidden lg:flex items-center gap-6 xl:gap-8 absolute left-1/2 -translate-x-1/2">
            <Link href="/" className="text-[12px] sm:text-[13px] md:text-[14px] font-medium text-gray-600 hover:text-black transition-colors whitespace-nowrap">Home</Link>
            <Link href="/#transport-partners" className="text-[12px] sm:text-[13px] md:text-[14px] font-medium text-gray-600 hover:text-black transition-colors whitespace-nowrap">Transport Partners</Link>
            <Link href="/#business" className="text-[12px] sm:text-[13px] md:text-[14px] font-medium text-gray-600 hover:text-black transition-colors whitespace-nowrap">Corporate Companies</Link>
            <Link href="/#faq" className="text-[12px] sm:text-[13px] md:text-[14px] font-medium text-gray-600 hover:text-black transition-colors whitespace-nowrap">FAQs</Link>
          </div>

          <Link
            href="mailto:info@joltride.com"
        ...
        <div className="max-w-[1200px] mx-auto flex flex-col md:flex-row gap-12 lg:gap-24 relative">
          ...
          <aside className="w-full md:w-64 flex-shrink-0 md:sticky md:top-32 md:h-[calc(100vh-140px)] overflow-y-auto hidden md:block">
            <div className="mb-6 flex items-center gap-2 text-gray-400">
              <LayoutList size={18} />
              <span className="text-sm font-semibold tracking-widest uppercase">Documentation</span>
            </div>
          ...
          <div className="md:hidden block w-full border-b border-gray-100 pb-6 mb-2">
            <span className="text-gray-400 text-xs font-bold uppercase tracking-widest block mb-4">Documentation Pages</span>
              ...
              {PAGES.map((page) => {
                ...
                return (
                  <Link
                    key={page.id}
                    href={page.href}
                    className={`flex items-center gap-2 px-3 py-2 rounded-full border text-xs font-medium transition-colors ${isActive
          ...
          <div className="flex-1 max-w-3xl w-full">
            ...
            <div className="mt-20 pt-10 border-t border-gray-100 grid grid-cols-1 sm:grid-cols-2 gap-4">
              {prevPage ? (
                <Link href={prevPage.href} className="group flex flex-col gap-2 p-5 rounded-xl border border-gray-100 bg-white hover:border-gray-300 hover:shadow-sm transition-all text-left w-full">
              ...
              {nextPage ? (
                <Link href={nextPage.href} className="group flex flex-col gap-2 p-5 rounded-xl border border-gray-100 bg-white hover:border-gray-300 hover:shadow-sm transition-all text-right w-full sm:col-start-2">
